...
naiveSuffixTree, depthfirst_to_suffix, suffix_to_depthfirst = treeStruct.build_tree(False, True)

def stoye_gusfield(node, depthfirst_to_suffix, testing):
    ...
    if node.type != "leaf":
        ...
        for index in range(depth_min, depth_max):
    ...
    for i in suffix_list:
        for j in suffix_list:
            if j != i and j == i + node.string_depth and node.sequence[i] != node.sequence[i + 2 * node.string_depth]:
                tandem_repeat = (i, 2* node.string_depth, 2)
                tandem_repeat_list.append(tandem_repeat)

    if node.children:
        for child in node.children:
            returned_tandem_repeats = stoye_gusfield(child, depthfirst_to_suffix, testing)
            tandem_repeat_list += returned_tandem_repeats
