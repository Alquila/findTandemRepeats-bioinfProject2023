...
def stoye_gusfield(node: Node, depthfirst_to_suffix, sequence, testing):
    ...
    #TODO - call LL(v) method to get small LL'(v) and large LL(v') and v'
    # replaces get suffix/leaf list below 
    ...
    # for loop runs trhough small LL'(v) and make corresponding checks
    # i loops through small LL'(v)
    # j loops through LL(v) with checks
    #   j == i + D(v) and S[i] != S[i+2D(v)] --> j is in small
    # j loops through LL(v') with checks ??? 
    #   j == i - D(v) and S[i-D(v)] != S[i+D(v)] --> j is in large
